...
$profile = Join-Path $tunnel "profiles\$script:RepoRelayTunnelProfile.yaml"
$secretDirectory = Join-Path $tunnel 'secrets'
$bridgeSecret = Join-Path $secretDirectory 'reporelay-bridge-secret.txt'
...
$updated = [regex]::Replace(
    $content,
    '(?m)^(\s*X-RepoRelay-Bridge-Secret:\s*)"?file:[^"\r\n]+"?\s*$',
...
if ($PSCmdlet.ShouldProcess($tunnel, 'Protect tunnel credentials and update profile references')) {
    ...
    [IO.File]::WriteAllText((Join-Path $history 'reporelay.protected.yaml'), $updated, [Text.UTF8Encoding]::new($false))
