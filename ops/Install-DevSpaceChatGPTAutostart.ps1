...
if ((Test-PathContains -Parent $workspace -Candidate $keyPath) -or (Test-PathContains -Parent $sourceRoot -Candidate $keyPath)) {
    throw 'The control-plane API key file must be outside both the approved repository and the DevSpace source tree.'
}
...
$launcher = Join-Path $PSScriptRoot 'Start-DevSpaceChatGPTFromConfig.ps1'
...
$task = New-ScheduledTask -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description 'Starts the loopback-only hardened DevSpace ChatGPT MCP stack for the current user.'
