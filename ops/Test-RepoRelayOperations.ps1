[CmdletBinding()]
param(
    [string]$RecycleScript = $env:REPORELAY_RECYCLE_SCRIPT
)
...
$fixture = Join-Path ([IO.Path]::GetTempPath()) ("RepoRelay-ops-test-" + [guid]::NewGuid().ToString('N'))
$backup = Join-Path ([IO.Path]::GetTempPath()) ("RepoRelay-ops-backup-" + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $initializer = Join-Path $PSScriptRoot 'Initialize-RepoRelayHandoff.ps1'
    ...
    Assert-True -Condition $agents.Contains('reporelay-handoff-v1') -Message 'handoff instructions were not appended'
    Assert-True -Condition $agents.Contains('`ready_for_review`') -Message 'Markdown code span was not preserved in handoff instructions'
    ...
    $hashesBefore = @{}
    foreach ($name in @('NEXT_TASK.md', 'REVIEW.md', 'RESULT.md', 'STATE.json')) {
    ...
    $nodeHome = (& (Get-NodeExecutable) -e "process.stdout.write(require('node:os').homedir())")
    foreach ($dangerous in @([IO.Path]::GetPathRoot($fixture), $nodeHome)) {
        $rejected = $false
...
'@
    ...
    $missingRuntime = Join-Path ([IO.Path]::GetTempPath()) ("RepoRelay-no-runtime-" + [guid]::NewGuid().ToString('N'))
    $stopOutput = & (Join-Path $PSScriptRoot 'Stop-RepoRelay.ps1') -RuntimeRoot $missingRuntime
    Assert-True -Condition (($stopOutput -join ' ') -like '*already stopped*') -Message 'stop was not idempotent when metadata was absent'

    $failedRuntimeRoot = Join-Path ([IO.Path]::GetTempPath()) ("RepoRelay-failed-runtime-" + [guid]::NewGuid().ToString('N'))
    ...
    $diagnosticsOutput = & (Join-Path $PSScriptRoot 'Get-RepoRelayDiagnostics.ps1') -RuntimeRoot $failedRuntimeRoot
    ...
    foreach ($path in @($fixture, $backup, $failedRuntimeRoot)) {
        if ($path -and (Test-Path -LiteralPath $path)) {
            if ($RecycleScript -and (Test-Path -LiteralPath $RecycleScript -PathType Leaf)) {
                ...
                Write-Output "Fixture preserved (set REPORELAY_RECYCLE_SCRIPT to recycle it): $path"
            }
        }
    }
}
