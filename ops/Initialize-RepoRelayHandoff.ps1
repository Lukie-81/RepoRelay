...
param(
    ...
    [string]$BackupRoot = (Join-Path ([Environment]::GetFolderPath('LocalApplicationData')) 'RepoRelay\onboarding-backups')
)
...
$marker = '<!-- reporelay-handoff-v1 -->'
$agentSection = @'
<!-- reporelay-handoff-v1 -->
## AI reviewer - Codex handoff

- AI reviewer independently inspects this repository through the constrained RepoRelay MCP tools. It may replace only `.ai-handoff/NEXT_TASK.md`, `.ai-handoff/REVIEW.md`, and `.ai-handoff/STATE.json`.
...
$templates = [ordered]@{
    ...
    'REVIEW.md' = "# Independent review`r`n`r`nStatus: pending`r`n`r`nAI reviewer writes its independent repository review here.`r`n"
    'RESULT.md' = "# Implementation result`r`n`r`nStatus: pending`r`n`r`nThe local implementer writes implementation and validation evidence here.`r`n"
    'STATE.json' = (@{
        schemaVersion = 1
        cycle = 0
...
if ($PSCmdlet.ShouldProcess($repository, 'Initialize constrained RepoRelay handoff files')) {
