...
$script:DevSpaceExpectedSourceRoot = [IO.Path]::GetFullPath((Split-Path -Parent $PSScriptRoot))
$script:DevSpaceDefaultRuntimeRoot = Join-Path ([Environment]::GetFolderPath('LocalApplicationData')) 'DevSpaceChatGPT'
$script:DevSpaceDefaultTunnelRoot = Join-Path $script:DevSpaceDefaultRuntimeRoot 'tunnel-client'
$script:DevSpaceTunnelProfile = 'devspace-chatgpt-test'
$script:DevSpaceRequiredTools = @(
    'list_files',
    'open_workspace',
    'read_file',
    'search_files',
...
function Assert-CanonicalSource {
    ...
    if (-not (Test-SamePath $sourceRoot $script:DevSpaceExpectedSourceRoot)) {
        throw "This operations script must run from its own source checkout: $script:DevSpaceExpectedSourceRoot"
    }
...
function Get-RuntimePaths {
    ...
    return [pscustomobject]@{
        ...
        DevSpaceStdout = Join-Path $fullRoot 'devspace.stdout.log'
        DevSpaceStderr = Join-Path $fullRoot 'devspace.stderr.log'
...
function Test-UnauthenticatedMcpRejected {
    ...
    $body = '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"devspace-ops","version":"1.0.0"}}}'
    try {
...
function Test-AuthenticatedMcpSurface {
    ...
    try {
        ...
        $headers = @{ Accept = 'application/json, text/event-stream'; 'X-DevSpace-Bridge-Secret' = $secret }
        $initialize = @{
            jsonrpc = '2.0'; id = 1; method = 'initialize'; params = @{
                protocolVersion = '2025-06-18'; capabilities = @{}; clientInfo = @{ name = 'devspace-ops'; version = '1.0.0' }
            }
        ...
        $expected = @($script:DevSpaceRequiredTools | Sort-Object)
...
function Find-CompatibleTunnelProcess {
    ...
    foreach ($process in @(Get-CimInstance Win32_Process -Filter "Name='tunnel-client.exe'" -ErrorAction SilentlyContinue)) {
        ...
        if ($commandLine -notmatch '(?i)\brun\b' -or $commandLine -notmatch "(?i)--profile\s+$([regex]::Escape($script:DevSpaceTunnelProfile))\b") { continue }
...
function Test-RecordedProcessIdentity {
    param([Parameter(Mandatory)]$Record, [Parameter(Mandatory)][ValidateSet('devspace', 'tunnel')][string]$Kind)
    ...
    $actualStart = (Get-Process -Id ([int]$Record.pid) -ErrorAction Stop).StartTime.ToUniversalTime()
