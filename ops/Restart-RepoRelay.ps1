...
$oldWasRunning = [bool]($oldMetadata -and $oldMetadata.status -eq 'running' -and (Test-RecordedProcessIdentity -Record $oldMetadata.reporelay -Kind reporelay))
$stopScript = Join-Path $PSScriptRoot 'Stop-RepoRelay.ps1'
$startScript = Join-Path $PSScriptRoot 'Start-RepoRelay.ps1'
...
try {
    ...
    if ($oldWasRunning -and $oldWorkspace) {
        ...
        try {
            ...
            Write-Warning 'The previous RepoRelay workspace was restored successfully.'
        } catch {
