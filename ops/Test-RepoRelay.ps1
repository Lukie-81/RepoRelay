...
if (-not (Test-SamePath ([string]$metadata.reporelay.cwd) $sourceRoot)) { throw 'Recorded RepoRelay CWD is not the canonical Tools source.' }
if ($WorkspaceRoot) {
...
if (-not (Test-RecordedProcessIdentity -Record $metadata.reporelay -Kind reporelay)) {
    throw 'Recorded RepoRelay process identity is not live or no longer matches.'
}
$port = [int]$metadata.port
Assert-LoopbackListener -ProcessId ([int]$metadata.reporelay.pid) -Port $port
...
try {
    foreach ($logProperty in @('stdoutLog', 'stderrLog')) {
        $logPath = [string]$metadata.reporelay.$logProperty
        if ((Test-Path -LiteralPath $logPath -PathType Leaf) -and (Read-TextFileShared $logPath).Contains($secret)) {
            throw 'A RepoRelay runtime log contains the bridge secret.'
        }
    }
} finally {
    $secret = $null
...
Write-Output "Listener: 127.0.0.1:$port (PID $($metadata.reporelay.pid))"
Write-Output "Tunnel: $tunnelStatus"
Write-Output "Tools: $($tools -join ', ')"
Write-Output 'READY FOR REVIEW: yes'
