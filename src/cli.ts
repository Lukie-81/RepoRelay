...
async function ensureConfigured(): Promise<void> {
  ...
  if (!input.isTTY || !output.isTTY) {
    throw new Error(
      [
        "RepoRelay is not configured and this terminal is non-interactive.",
        "",
        "Run:",
        "  reporelay init",
        "",
...
async function runInit({ force }: { force: boolean }): Promise<void> {
  ...
  if (!force && files.configExists && files.authExists) {
    prompts.log.info(`RepoRelay is already configured at ${files.dir}`);
    prompts.log.info("Run `reporelay init --force` to update it.");
    return;
  }

  try {
    prompts.intro("RepoRelay setup");
    ...
    prompts.note(
      [
        "RepoRelay needs a public base URL so an MCP client can reach this server.",
    ...
    prompts.outro("Run `reporelay serve` to start the MCP server.");
  } catch (error) {
...
async function serve(): Promise<void> {
  ...
  const httpServer = app.listen(config.port, config.host, () => {
    console.log(`reporelay listening on http://${config.host}:${config.port}/mcp`);
...
async function runDoctor(): Promise<void> {
  ...
  try {
    ...
    console.log(`Bridge authentication: ${config.chatgptBridgeAuth ? "enabled" : "disabled"}`);
...
function runConfigCommand(args: string[]): void {
  ...
  if (key !== "publicBaseUrl") {
    throw new Error("Only `reporelay config set publicBaseUrl <url|null>` is supported right now.");
  }
...
function printHelp(): void {
  console.log(
    [
      ...
      "  reporelay config set publicBaseUrl <url|null>",
      "  reporelay agents ls       List subagent sessions",
      "  reporelay agents run <profile-or-provider-or-id> [--model <model>] <prompt>",
      "  reporelay agents show <id>",
      "  reporelay -v, --version   Print the installed version",
      "",
      "For temporary tunnels:",
      "  DEVSPACE_PUBLIC_BASE_URL=https://example.trycloudflare.com reporelay serve",
    ].join("\n"),
  );
}
...
function printAgentsHelp(): void {
  console.log(
    [
      ...
      "Usage:",
      "  reporelay agents ls",
      "  reporelay agents run <profile-or-provider-or-id> [--model <model>] [--thinking <level>] <prompt>",
      "  reporelay agents show <id>",
    ].join("\n"),
  );
}
...
function printVersion(): void {
  ...
  if (typeof packageJson.version !== "string") {
    throw new Error("Unable to read RepoRelay package version.");
  }
...
function assertSupportedNode(): void {
  ...
  throw new Error(
    [
      `RepoRelay requires Node ${SUPPORTED_NODE_RANGE}.`,
