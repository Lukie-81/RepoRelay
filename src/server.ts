...
export const CHATGPT_BRIDGE_HEADER = "x-devspace-bridge-secret";
...
function serverInstructions(config: ServerConfig): string {
  ...
  if (config.toolMode === "chatgpt-review") {
    const handoffInstruction = config.handoffWritesEnabled
      ? " Dedicated handoff tools may replace only NEXT_TASK.md, REVIEW.md, and STATE.json under .ai-handoff; they never accept a destination path."
      : " Handoff writes are disabled; this connection is strictly read-only.";
    return `Use RepoRelay only to independently inspect the approved repository. Call open_workspace once, then use list_files, read_file, and search_files. No shell, process, general write, edit, delete, patch, artifact, worktree, skill, or subagent capability is available.${handoffInstruction}`;
  }
...
export function registerChatgptReviewTools(
  server: McpServer,
  config: ServerConfig,
  workspaces: WorkspaceRegistry,
): void {
  registerAppTool(
    ...
    "open_workspace",
    {
      title: "Open approved review workspace",
      description:
        "Open one existing repository inside the configured approved roots. This profile never creates directories or worktrees.",
      inputSchema: {
        path: z.string().describe("Existing absolute repository path inside an approved root."),
      },
      outputSchema: {
    ...
    async ({ path }) => {
      const { workspace, agentsFiles, availableAgentsFiles } = await workspaces.openWorkspace(path);
      const loadedAgentsFiles = agentsFiles.map((file) => ({
        path: formatAgentsPath(file.path, workspace.root),
        content: file.content,
      }));
      const availableAgentFiles = availableAgentsFiles.map((file) => ({
        path: formatAgentsPath(file.path, workspace.root),
      }));
      const result = `Opened read-confined workspace ${workspace.id} at ${workspace.root}.`;
      return {
        content: [textBlock([
          result,
          loadedAgentsFiles.length > 0
            ? `Loaded instructions: ${loadedAgentsFiles.map((file) => file.path).join(", ")}`
            : undefined,
          availableAgentFiles.length > 0
            ? `Nested instructions: ${availableAgentFiles.map((file) => file.path).join(", ")}`
            : undefined,
        ].filter(Boolean).join("\n"))],
        structuredContent: {
          ...
          availableAgentsFiles: availableAgentFiles,
          result,
        },
      };
    },
  ...
  registerAppTool(
    ...
    "list_files",
    {
      title: "List approved repository files",
      description: "List one real directory inside an open approved workspace. Links and junctions are reported as blocked and never followed.",
      inputSchema: {
        workspaceId: z.string(),
    ...
    async ({ workspaceId, path }) => {
      ...
      const result = entries.map((entry) => `${entry.type}\t${entry.path}`).join("\n");
      return {
        content: [textBlock(result || "Directory is empty.")],
  ...
  registerAppTool(
    ...
    "read_file",
    {
      title: "Read approved repository file",
      description: "Read one regular file whose canonical filesystem target remains inside the open workspace. Links, junctions, hard links, and outside paths fail closed.",
      inputSchema: {
        workspaceId: z.string(),
    ...
    async ({ workspaceId, path }) => {
      const workspace = workspaces.getWorkspace(workspaceId);
      const result = await readReviewTextFile(workspace.root, path);
  ...
  registerAppTool(
    ...
    "search_files",
    {
      title: "Search approved repository files",
      description: "Case-insensitive literal text search across regular files inside the open workspace. Links and junctions are never followed.",
      inputSchema: {
        workspaceId: z.string(),
    ...
    async ({ workspaceId, query, path, maxResults }) => {
      ...
      const result = matches.map((match) => `${match.path}:${match.line}: ${match.text}`).join("\n");
      return {
        content: [textBlock(result || "No matches.")],
        structuredContent: { result },
      };
    },
  );
  ...
  const registerHandoffWriter = (
    ...
    registerAppTool(
      ...
      name,
      {
        ...
        inputSchema: {
          workspaceId: z.string(),
          content: z.string().describe("Complete replacement content. The destination path is fixed by the tool."),
        },
        outputSchema: resultOutputSchema(),
      ...
      async ({ workspaceId, content }) => {
        const workspace = workspaces.getWorkspace(workspaceId);
        const path = await writeHandoffDocument(workspace.root, document, content);
        const result = `Updated ${path}.`;
        return { content: [textBlock(result)], structuredContent: { result } };
      },
    );
  };

  registerHandoffWriter(
    "write_next_task",
...
function createMcpServer(
  ...
  const server = new McpServer(
    {
      ...
      version: config.toolMode === "chatgpt-review" ? "1.1.1" : "0.1.0",
  ...
  if (config.toolMode === "chatgpt-review") {
    registerChatgptReviewTools(server, config, workspaces);
    return server;
  }
...
export function createServer(
  ...
  assertChatgptBridgeSafety(config);
  ...
  const oauthProvider = config.chatgptBridgeAuth
    ? undefined
    : new SingleUserOAuthProvider(config.oauth, mcpUrl, config.stateDir);
  const mcpAuthentication = config.chatgptBridgeAuth
    ? (req: Request, res: Response, next: NextFunction) => {
      const headerCount = req.rawHeaders.reduce((count, name, index) => (
        index % 2 === 0 && name.toLowerCase() === CHATGPT_BRIDGE_HEADER
          ? count + 1
          : count
      ), 0);
      const supplied = req.get(CHATGPT_BRIDGE_HEADER);
      const expected = config.chatgptBridgeSecret;
      const suppliedDigest = createHash("sha256").update(supplied ?? "", "utf8").digest();
      const expectedDigest = createHash("sha256").update(expected ?? "", "utf8").digest();

      if (headerCount !== 1 || !supplied || !expected || !timingSafeEqual(suppliedDigest, expectedDigest)) {
  ...
  const workspaceStore = config.chatgptBridgeAuth
    ? undefined
    : createWorkspaceStore(config.stateDir);
  ...
  app.all("/mcp", mcpAuthentication, mcpJsonBodyParser, async (req, res) => {
    const requestId = res.locals.requestId as string | undefined;
    const sessionId = req.header("mcp-session-id");
    const initializeRequest = req.method === "POST" && isInitializeRequest(req.body);

    if (
      !config.chatgptBridgeAuth
      && (!req.auth?.resource || !checkResourceAllowed({
...
if (await isMainModule()) {
  ...
  const httpServer = app.listen(config.port, config.host, () => {
    ...
    console.log(config.chatgptBridgeAuth
      ? "auth: tunnel-injected bridge secret required"
