...
export class WorkspaceRegistry {
  ...
  async openWorkspace(input: string | OpenWorkspaceInput): Promise<WorkspaceContext> {
    ...
    if (this.config.toolMode === "chatgpt-review" && mode !== "checkout") {
      throw new Error("The chatgpt-review profile does not create or open managed worktrees.");
    }
  ...
  getWorkspace(workspaceId: string): Workspace {
    ...
    if (this.config.toolMode === "chatgpt-review") {
      throw new Error(`Unknown workspaceId: ${workspaceId}. Call open_workspace again after a server restart.`);
    }
  ...
  private async createWorkspaceContext(input: {
    ...
    const workspace: Workspace = {
      ...
      agentProfiles: this.config.toolMode === "chatgpt-review"
        ? []
        : await loadLocalAgentProfiles(this.config, input.root),
      activatedSkillDirs: new Set(),
    };
  ...
  private async loadInitialAgentsFiles(root: string): Promise<LoadedAgentsFile[]> {
    if (this.config.toolMode === "chatgpt-review") {
      const instructionPaths = await findReviewInstructionFiles(root);
      const rootInstructionPaths = instructionPaths.filter((path) => dirname(resolve(root, path)) === root);
      return Promise.all(rootInstructionPaths.map(async (path) => ({
        path: resolve(root, path),
        content: await readReviewTextFile(root, path),
      })));
    }
  ...
  private async findAvailableAgentsFiles(
    ...
    if (this.config.toolMode === "chatgpt-review") {
      ...
      const paths = await findReviewInstructionFiles(root);
      return paths
        .map((path) => ({ path: resolve(root, path) }))
        .filter((file) => !loadedPaths.has(file.path));
    }
